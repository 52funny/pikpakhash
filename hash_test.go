...
func BenchmarkHashBuf128K(b *testing.B) {
	ph := NewPikPakHashWithBuf(DefaultCalcRoutines, 1<<17)
	for i := 0; i < b.N; i++ {
		ph.HashFromPath("./b")
	}
}
...
func BenchmarkHashBuf256K(b *testing.B) {
	ph := NewPikPakHashWithBuf(DefaultCalcRoutines, 1<<18)
	for i := 0; i < b.N; i++ {
		ph.HashFromPath("./b")
	}
}
func BenchmarkHashBuf512K(b *testing.B) {
	ph := NewPikPakHashWithBuf(DefaultCalcRoutines, 1<<19)
	for i := 0; i < b.N; i++ {
		ph.HashFromPath("./b")
	}
}
...
func BenchmarkHashBuf1M(b *testing.B) {
	ph := NewPikPakHashWithBuf(DefaultCalcRoutines, 1<<20)
	for i := 0; i < b.N; i++ {
		ph.HashFromPath("./b")
	}
}
